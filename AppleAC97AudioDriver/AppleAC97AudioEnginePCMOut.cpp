...
#define kConfigArraySize  6

#define CLASS AppleAC97AudioEnginePCMOut
#define super AppleAC97AudioEngine

OSDefineMetaClassAndStructors( AppleAC97AudioEnginePCMOut,
...
bool CLASS::initAudioConfigurations( void )
{
    ...
    // Generate configurations for 2, 4 and 6 channel PCM audio.

    if (!fConfigArray) return false;
    fConfigArray->flushCollection();

    for (int index = 0; index < kAudioConfigCount; index++)
    {
        int numChannels = (index+1) * 2;
    ...
    if (config == 0)
    {
        DebugLog("%s: no PCM Out configuration\n", getName());
        return false;
    }
#else
...
bool CLASS::initAudioStreams( void )
{
         ...
         i++)
    {
        ...
        DebugLog("%s: added %u-ch/%u format to output stream\n",
    ...
    if (fConfigActive)
    {
        ...
        DebugLog("%s: set output stream format to %lu-ch, tag %lu\n",
