...
IOByteCount CLASS::getDMAEngineHardwarePointer( IOAC97DMAEngineID engine )
{
    ...
    dma = &fDMAState[engine];
	long temp = ACC_BM0_PNTR + (engine * 4);
	position = ACC_READ32(temp);
                                            ...
                                            void *                param )
{
    ...
    {
        dma->sampleMemoryPhysAddr = dma->sampleMemory->getPhysicalSegment((IOByteCount)0, (IOByteCount*)&size);
    }
    ...
    if (dma->prdMemory == 0)
    {
        ...
        dma->prdPhysAddr = dma->prdMemory->getPhysicalSegment((IOByteCount)0,(IOByteCount*)&size);
    ...
    if (dma->prdBufferSize != kDMABufferBytes)
    {
        ...
        for ( UInt32 i = 0; i < kDMABufferCount; i++ )
        {
            ...
            phys = dma->sampleMemory->getPhysicalSegment(
                                (IOByteCount)i * kDMABufferBytes, (IOByteCount*)&size);
            IOAC97Assert(phys != 0);
