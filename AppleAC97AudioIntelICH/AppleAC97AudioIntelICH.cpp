...
public:
    ...
    virtual IOByteCount getBaseAddress( void ) const = 0;
...
public:
    ...
    virtual IOByteCount getBaseAddress( void ) const;
...
{
    IOByteCount     remain;    // number of samples left in current buffer
                                            ...
                                            void *                param )
{
    ...
    if (dma->bdMemory == 0)
    {
        ...
        dma->bdPhysAddr = dma->bdMemory->getPhysicalSegment((IOByteCount)0, (IOByteCount*)&size);
    ...
    if (dma->bdBufferSize != bufferBytes)
    {
        ...
        for ( IOByteCount i = 0; i < kDMABufferCount; i++ )
        {
            IOPhysicalAddress phys;

            phys = dma->sampleMemory->getPhysicalSegment(
                                (IOByteCount)i * bufferBytes, (IOByteCount*)&size);
            IOAC97Assert(phys != 0);
...
#pragma mark -
#pragma mark ••• Low-Level Hardware Access •••
#pragma mark -
...
IOByteCount AppleAC97AudioIntelMemoryReg::getBaseAddress( void ) const
{
    return (IOByteCount) fMemBase;
}
...
IOByteCount AppleAC97AudioIntelIOReg::getBaseAddress( void ) const
{
    return fIOBase;
}
